    ...
    async execute(message, args, client) {
        ...
        try {
            ...
            const testEmbed = {
                ...
                description: 'This channel has been set as the welcome channel.',
                fields: [
                    { name: 'Configured By', value: message.author.tag, inline: true },
                    { name: 'Server', value: message.guild.name, inline: true }
                ],
                timestamp: new Date(),
                footer: { text: 'New members will receive a welcome message here!' }
            };
            channel.send({ embeds: [testEmbed] });
            
        } catch (error) {
            console.error('Set welcome channel error:', error);
            message.reply({ embeds: [embedBuilder.error('Failed', 'An error occurred while setting the welcome channel.')] });
        }
    }
};